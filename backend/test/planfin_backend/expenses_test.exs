defmodule PlanfinBackend.ExpensesTest do
  ...
  defp setup_group_with_period_and_subcategory do
    {user, group, period} = setup_group_with_period()

    {:ok, category} = Categories.create_category(group.id, %{name: "Food"})
    {:ok, subcategory} = Categories.create_subcategory(category, %{name: "Restaurant"})

    {user, group, period, subcategory}
  end
  ...
  describe "create_expense/3" do
    test "creates expense with active period and valid date, records authorship" do
      {user, group, _period} = setup_group_with_period()

      assert {:ok, %Expense{} = expense} =
               Expenses.create_expense(group.id, user.id, valid_expense_attrs())
    ...
    test "returns {:error, :no_active_period} when group has no active period" do
      {user, group} = user_with_group_fixture()
    ...
    test "returns {:error, :date_out_of_range} when date is before period start" do
      ...
      assert {:error, :date_out_of_range} =
               Expenses.create_expense(
                 group.id,
                 user.id,
                 valid_expense_attrs(%{date: ~D[2026-03-31]})
               )
    end
    ...
    test "returns {:error, :date_out_of_range} when date is after period end" do
      ...
      assert {:error, :date_out_of_range} =
               Expenses.create_expense(
                 group.id,
                 user.id,
                 valid_expense_attrs(%{date: ~D[2026-05-01]})
               )
    end
    ...
    test "returns changeset error when amount is zero" do
      ...
      assert {:error, changeset} =
               Expenses.create_expense(
                 group.id,
                 user.id,
                 valid_expense_attrs(%{amount: Decimal.new("0")})
               )
    ...
    test "associates subcategory when subcategory_id is provided" do
      {user, group, _period, subcategory} = setup_group_with_period_and_subcategory()
  ...
  describe "delete_expense/2" do
    test "deletes the expense and returns {:ok, expense}" do
      {user, group, _period} = setup_group_with_period()
      {:ok, expense} = Expenses.create_expense(group.id, user.id, valid_expense_attrs())

      assert {:ok, %Expense{}} = Expenses.delete_expense(group.id, expense.id)
      refute Repo.get(Expense, expense.id)
    end

    test "returns {:error, :not_found} when expense belongs to another group" do
      {u1, g1, _period1} = setup_group_with_period()
      {_u2, g2} = user_with_group_fixture()

      {:ok, expense} = Expenses.create_expense(g1.id, u1.id, valid_expense_attrs())

      assert {:error, :not_found} = Expenses.delete_expense(g2.id, expense.id)
    ...
    test "returns {:error, :not_found} for non-existent expense" do
      ...
      assert {:error, :not_found} = Expenses.delete_expense(group.id, fake_id)
    end
  end
  ...
  describe "list_expenses_by_day/2" do
    test "returns expenses for the given budget_day ordered by inserted_at desc" do
      ...
      {:ok, e1} =
        Expenses.create_expense(group.id, user.id, valid_expense_attrs(%{date: ~D[2026-04-05]}))
      ...
      expenses = Expenses.list_expenses_by_day(group.id, budget_day.id)
    ...
    test "does not return expenses from a different budget_day" do
      ...
      {:ok, _e2} =
        Expenses.create_expense(group.id, user.id, valid_expense_attrs(%{date: ~D[2026-04-10]}))
    ...
    test "preloads subcategory with category" do
      ...
      budget_day = Repo.get_by!(BudgetDay, date: ~D[2026-04-05])
      [expense] = Expenses.list_expenses_by_day(group.id, budget_day.id)
  ...
  describe "list_expenses_by_period/2" do
    test "returns expenses grouped by date with total, ordered by date desc" do
      ...
      grouped = Expenses.list_expenses_by_period(group.id, period.id)
    ...
    test "does not return expenses from another group's period" do
      {u1, g1, period1} = setup_group_with_period()

      {_u2, g2} = user_with_group_fixture()
      {:ok, _period2} = Periods.create_period(g2.id, @period_attrs)

      {:ok, _} = Expenses.create_expense(g1.id, u1.id, valid_expense_attrs())

      grouped = Expenses.list_expenses_by_period(g2.id, period1.id)
      assert grouped == []
    end

    test "returns empty list when there are no expenses" do
      {_user, group, period} = setup_group_with_period()

      grouped = Expenses.list_expenses_by_period(group.id, period.id)
      assert grouped == []
    end

    test "multiple members of same group share expense visibility with authorship" do
      {u1, group, _period} = setup_group_with_period()
      u2 = user_fixture()
      _ = join_group_fixture(u2, group)

      {:ok, _} =
        Expenses.create_expense(
          group.id,
          u1.id,
          valid_expense_attrs(%{date: ~D[2026-04-05]})
        )
      ...
      # Fetch budget_day so list_expenses_by_day works
      budget_day = Repo.get_by!(BudgetDay, date: ~D[2026-04-05])
      expenses = Expenses.list_expenses_by_day(group.id, budget_day.id)

      assert length(expenses) == 2
      authors = expenses |> Enum.map(& &1.created_by.id) |> Enum.sort()
      assert authors == Enum.sort([u1.id, u2.id])
    end
  end

  describe "get_expense!/2" do
    test "returns the expense when it belongs to the group" do
      {user, group, _period} = setup_group_with_period()
      {:ok, expense} = Expenses.create_expense(group.id, user.id, valid_expense_attrs())

      found = Expenses.get_expense!(group.id, expense.id)
      assert found.id == expense.id
    end

    test "raises when expense belongs to another group" do
      {u1, g1, _period1} = setup_group_with_period()
      {_u2, g2} = user_with_group_fixture()
      ...
      assert_raise Ecto.NoResultsError, fn ->
        Expenses.get_expense!(g2.id, expense.id)
      end
    end
  end
end
