defmodule PlanfinBackend.Accounts.UserToken do
  use Ecto.Schema
  import Ecto.Query
  alias PlanfinBackend.Accounts.UserToken
  ...
  @doc """
  Builds an API token (unhashed) for mobile/API clients.
  Returns `{encoded_token, user_token}`.
  """
  def build_api_token(user) do
    token = :crypto.strong_rand_bytes(@rand_size)
    encoded = Base.url_encode64(token, padding: false)
    hashed_token = :crypto.hash(@hash_algorithm, token)
    {encoded, %UserToken{token: hashed_token, context: "api", user_id: user.id}}
  end
